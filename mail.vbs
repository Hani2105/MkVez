...
Set file_cc = fso.OpenTextFile(strCurDir & "\cc.txt", 1)
...
Set file_cc_size = fso.GetFile(strCurDir & "\cc.txt")

If (CINT(file_cc_size.Size)>0) then
content_cc = file_cc.ReadAll
else
content_cc=""
end If
...
If (fso.FileExists(strCurDir & "\process.txt")) Then
fso.DeleteFile strCurDir & "\process.txt"
end If
